...
# - Keep "set-like" config lines by editing airflow.cfg (cfg_set)
...
# --------------------------
# Helper: cfg_set (MUST be defined before [7/9] uses it)
# --------------------------
cfg_set() {
  local section="$1"
  local key="$2"
  ...
  if [ ! -f "$cfg" ]; then
    echo "錯誤: 找不到 $cfg，請先生成 airflow.cfg（先跑 airflow version 生成）"
    return 1
  fi
  ...
  # 若 key 已存在於 section 區塊內 => replace，否則 insert
  if awk -v sec="[$section]" -v k="$key" '
      ...
      END{exit !found}
    ' "$cfg"; then
    sed -i "/^\[$section\]/,/^\[/{s|^${key}[[:space:]]*=.*|${key} = ${value}|}" "$cfg"
  else
...
# [7/9] airflow.cfg
# --------------------------
echo ">>> [7/9] 產生並設定 airflow.cfg（保留 set 語意）..."
runuser -u "$AIRFLOW_USER" -- bash -lc "
  set -euo pipefail
  source '$VENV_DIR/bin/activate'
...
echo "- 重要修正：不再 chown /opt/airflow 全目錄，避免 postgres_data 權限被改壞"
