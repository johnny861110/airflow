...
# - Auto-fix: "Skipping initialization" by wiping volume once if needed
# - Wait for Postgres ready (no blind sleep)
# - Constraint-based pip install
# - Robust airflow.cfg edits via `airflow config set`
...
# NOTE: 先 chown airflow 相關目錄，postgres_data 會在 DB 段落獨立處理
...
# --------------------------
# [5/9] Postgres/Redis (包含 Skipping initialization 自動修復)
# --------------------------
...
fix_skipping_initialization_once() {
  echo "  - 偵測到 Skipping initialization，進行一次性清空 postgres_data 並重建..."
...
# 如果沒 ready，直接看 log
if [ "$READY" -ne 1 ]; then
  echo "  - Postgres 尚未 ready，檢查是否 Skipping initialization..."
...
# 再次確認 DB 真存在（避免只 ready 但 airflow DB 沒建立）
if ! podman exec airflow-postgres psql -U airflow -d template1 -tAc "SELECT 1 FROM pg_database WHERE datname='airflow';" | grep -q 1; then
  echo "  - Postgres ready 但 airflow DB 不存在，嘗試 auto-fix Skipping initialization..."
  fix_skipping_initialization_once
  echo "  - 重建後等待 Postgres ready（最多 120 秒）..."
  READY="$(wait_pg_ready)"
  if [ "$READY" -ne 1 ]; then
...
echo "- Auto-fix: 若出現 Skipping initialization 會清空 postgres_data 一次並重建"
echo "- Pip: 使用 constraints 安裝避免 provider 衝突"
echo "- airflow config: 使用 airflow config set，並 unset core.auth_manager 避免 FAB 缺件"
echo "驗證："
echo "  podman exec airflow-postgres psql -U airflow -d template1 -c \"\\l\""
